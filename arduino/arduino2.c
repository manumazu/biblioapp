...
int sensorCols = 0;//for columns
int outputCols = 0;
int currentCol = 0;
int sensorRow = 0;//for lines
int outputRow = 0;
int currentRow = 0;
...
void getLedAddress() {
  sensorCols = analogRead(A0);
  // map it to the range pix of the analog out:
  outputCols = map(sensorCols, 0, 1023, 0, 11);
  
  sensorRow = analogRead(A1);
  // map it to the range lines of the analog out:
  outputRow = map(sensorRow, 0, 1023, 1, 2);  
